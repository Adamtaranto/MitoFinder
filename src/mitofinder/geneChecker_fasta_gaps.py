        ...
        with open("important_features.blast.out", "w") as blastResultFile:
            if usedOwnGenBankReference == True:  # using a personal genbank reference
                command = (
                    ...
                    + " -evalue "
                    + str(blasteVal)
                    + " -outfmt 6 -num_threads 2 -query_gencode "
                    + str(organismType)
                    + " -seg no"
                )  # call BLAST with XML output
            else:  # using a non personal genbank reference
                        ...
                        if featureName in listOfImportantFeatures:
                            ...
                            if (
                                ...
                                if featureName in listOfPresentFeatures:
                                    ...
                                    mainFeatureFound = listOfPresentFeatures[
                                        mainFeatureName
                                    ]
                                    ...
                                    if (
                                        (
                                            abs(
                                                startBase
                                                - mainFeatureFoundAlignment.endBase
                                            )
                                            <= float(gapsize)
                                            or abs(
                                                endBase
                                                - mainFeatureFoundAlignment.startBase
                                            )
                                            <= float(gapsize)
                                        )
                                        and (
                                            abs(
                                                ...
                                                + abs(endBase - startBase)
                                            )
                                            <= len(targetFeature * 3) + 33
                                        )
                                        and intron == 1
                                        and numt == 0
                                    ):
                                        logging.info("%s is split" % featureName)
                                        if not (
                                            startBase
                                            > mainFeatureFoundAlignment.startBase
                                            and endBase
                                            ...
                                            dico_feature[featureName] = (
                                                dico_feature.get(featureName) + 1
                                            )
                                            featureName += "_" + str(
                                                dico_feature.get(featureName)
                                            )
                                    ...
                                    if (
                                        ...
                                        logging.info("%s is duplicated." % featureName)
                                        if not (
                                            startBase
                                            > mainFeatureFoundAlignment.startBase
                                            and endBase
                    ...
                    if float(str(hsp.ident_num) + ".00") / float(
                        ...
                        if featureName in listOfImportantFeatures:
                            ...
                            if (
                                ...
                                if featureName in listOfPresentFeatures:
                                    logging.info(
                                        "%s is split or duplicated" % featureName
                                    )
                                    mainFeatureName = featureName
                                    mainFeatureFound = listOfPresentFeatures[
                                        mainFeatureName
                                    ...
                                    if (
                                        ...
                                        if not (
                                            ...
                                            if featureName not in listOfSplits:
                                                listOfSplits.append(featureName)
                                            featureName += "_" + str(
                                                listOfPresentFeatures.keys().count(
                                                    featureName
                                                )
                                                + 1
                                            )
