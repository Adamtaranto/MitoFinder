...
def genbankOutput(
    ...
    for thisFeatureAlignment in listOfFeaturesToOutput:
        if not (
            "trn" in thisFeatureAlignment.seq2.lower()
            or "rrn" in thisFeatureAlignment.seq2.lower()
            ...
            if main_feature_type == "gene":
                ...
                try:
                    ...
                    try:
                        ...
                        if strandToOutput == -1:
                            while (
                                not coding_dna_Forward.endswith(stopCodons)
                                ...
                                try:
                                    ...
                                    coding_dna_Backward = (
                                        coding_dna_Backward.reverse_complement()
                                    )
                                    coding_dna_Forward = Seq(
                                        str(
                                            finalResults.seq[
                                    ...
                                    coding_dna_Forward = (
                                        coding_dna_Forward.reverse_complement()
                                    )
                                    """print(str(strandToOutput))
                    ...
                    try:
                        ...
                        if strandToOutput == 1:
                            while (
                                not coding_dna_Forward.endswith(stopCodons)
                                and not coding_dna_Backward.endswith(stopCodons)
                                and dico_gene.get(gene) == dico_intron.get(gene)
                                and n < nWalkStop
                                and endBase + (3 * (n + 1)) <= len(finalResults)
                            ):
                                try:
                                    n += 1
                                    coding_dna_Backward = Seq(
                    ...
                    if strandToOutput == 1:
                        ...
                        coding_dna_Translation = (
                            coding_dna.reverse_complement().translate(
                                table=translationTable
                            )
                        )
                ...
                cds_feature = SeqFeature.SeqFeature(
                    main_feature_location, type="CDS", qualifiers=cds_qualifiers
                )
                main_feature = SeqFeature.SeqFeature(
                    main_feature_location,
                    type=main_feature_type,
