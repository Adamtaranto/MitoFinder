...
def geneCheck(
    ...
    print(
        "Checking genes, tRNAs and rRNAs from reference with organismType=%s..."
        % organismType
    )
        ...
        for feature in record.features:
            if feature.type.lower() == "cds":
                ...
                if featureName in listOfImportantFeatures:
                    featureName += "_" + str(
                        listOfImportantFeatures.keys().count(featureName) + 1
                    )
                    ...
                    importantFeaturesFile.write(
                        str(
                            feature.extract(record).seq.translate(
                                table=organismType, to_stop=True
                            )
                        )
                        + "\n"
                    )
                    print(
                        "WARNING: reference did not give a CDS translation for %s. Creating our own from refSeq."
                        % featureName
                    )
    ...
    if blastFolder == "installed":
        command = (
            "formatdb -i important_features.fasta -p T"  # need to formatdb refseq first
        )
    else:
        command = (
            blastFolder + "/bin/makeblastdb -in important_features.fasta -dbtype prot"
    ...
    with open("important_features.blast.xml", "w") as blastResultFile:
        if usedOwnGenBankReference == True:  # using a personal genbank reference
            if blastFolder == "installed":
                command = (
                    "blastall -p blastx -d important_features.fasta -i"
                    + resultFile
                    + " -e 0.1 -m 7"
                ...
                command = (
                    blastFolder
                    + "/bin/blastx -db important_features.fasta -query "
                    + resultFile
                    + " -evalue 0.1 -outfmt 5 -num_threads 2 -query_gencode "
                    + str(organismType)
                )  # call BLAST with XML output
            ...
            for (
                ...
                if featureName in listOfImportantFeatures:
                    ...
                    if featureName in listOfPresentFeatures:
                        ...
                        if (
                            abs(startBase - mainFeatureFoundAlignment.endBase) < 100
                            or abs(endBase - mainFeatureFoundAlignment.startBase) < 100
                        ) or (
                            mainFeatureFoundAlignment.startBase <= 60
                            and endBase >= len(refSeq.seq) - 60
                        ):
                            print("%s is split or duplicated." % featureName)
                            if not (
                                startBase > mainFeatureFoundAlignment.startBase
                                and endBase < mainFeatureFoundAlignment.endBase
                            ):
                                if featureName not in listOfSplits:
                                    listOfSplits.append(featureName)
                                featureName += "_" + str(
                                    listOfPresentFeatures.keys().count(featureName) + 1
                                )
    ...
    if blastFolder == "installed":
        ...
        command = (
            blastFolder + "/bin/makeblastdb -in " + resultFile + " -dbtype nucl"
        )  # need to formatdb refseq first
        ...
        if (
            ...
            if blastFolder == "installed":
                command = (
                    "blastall -p blastn -d "
                    + resultFile
                    + " -i important_features.fasta -e 4.0 -m 7"
                )  # call BLAST with XML output
            else:
                command = (
                    blastFolder
                    ...
                    + resultFile
                    + " -query important_features.fasta -evalue 4.0 -outfmt 5 -num_threads 2 -word_size 8 -perc_identity "
                    + str(cutoffEquality)
            ...
            if blastFolder == "installed":
                command = (
                    "blastall -p blastn -d "
                    + resultFile
                    + " -i important_features.fasta -e 6.0 -m 7"
                )  # call BLAST with XML output
            else:
                command = (
                    blastFolder
                    + "/bin/blastn -task blastn -db "
                    + resultFile
                    + " -query important_features.fasta -evalue 6.0 -outfmt 5 -num_threads 2 -word_size 8 -perc_identity "
                    + str(cutoffEquality)
                    + " -max_hsps 5 -gapextend 2 -gapopen 2"
                )  # call BLAST with XML output
        args = shlex.split(command)
    ...
    for qresult in blastparse:  # in each query, let's look for a good hit
        for (
            hsp
        ) in qresult.hsps:  # hsp object checking, this contains the alignment info
            ...
            if alignLen >= len(targetFeature) * alignCutOff:
                ...
                alignment.seqFound = refSeq.seq[startBase:endBase]
                listOfPresentFeatures[featureName] = (
                    listOfImportantFeatures[featureName],
                    alignment,
                    featureFrame == -1,
                )
                break
...
if __name__ == "__main__":
    if sys.argv[1] == "-h" or sys.argv[1] == "--help":
        print(
            "Usage: genbank_reference fasta_file output_file organism_type(integer, default=2) alignCutOff(float, default=0.45) coveCutOff(7)"
        )
        ...
        assemblyCheck = tRNAscanChecker.tRNAscanCheck(
            resultFile, True, False, organismType, coveCutOff
        ...
        finalResults = genbankOutput.genbankOutput(
            outputFile, resultFile, listOfFeaturesToOutput, False, 900
        )
        ...
        with open(outputFile + ".tbl", "w") as outputSeqIn:
            ...
            for gbkFeature in finalResults.features:
                if (
                    gbkFeature.location.strand == 1
                    or gbkFeature.location.strand == None
                ):
                    outputSeqIn.write(
        ...
        if (
            ("TRNF" in presentFeatures)
            or ("tRNA-Phe" in presentFeatures)
            ...
            with open(resultOrderedGbFile, "w") as outputResult:  # create the file!
                ...
                count = SeqIO.write(
                    orderedFinalResults,
                    resultOrderedGbFile.replace(".gb", ".fasta"),
                    "fasta",
                )
            ...
            print(
                "Since tRNA-Phe couldn't be found, ordered genbank file wasn't created."
            )
