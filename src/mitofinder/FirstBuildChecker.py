from mitofinder import tRNAscanChecker, circularizationCheck, geneChecker
...
def checkResults(
    ...
    sizeToLook,
    refSeqFile=None,
    cutoffValue=(2500, 18000),
    blasteVal=10.0,
    blastHitSizePercentage=0.625,
    usingSOAP=True,
    numberOfReadGroups=1,
    buildCloroplast=False,
    skipTrnaScan=False,
    circularSize=40,
    circularOffSet=220,
    cutoffEquality=0.625,
    organismType=2,
    blastFolder="installed",
    noExtension=False,
    ignoreFirstBuildChecks=False,
    coveCutOff=8,
    buildBacteria=False,
    buildArchea=False,
    validContigs=1,
    ...
    if buildCloroplast == True:
        ...
        if buildBacteria == True:
            refSeqFileForGenes = os.path.join(
                module_dir, "references/bacteria.gb"
            )  # some bacterial DNA
        elif buildArchea == True:
            refSeqFileForGenes = os.path.join(
                module_dir, "references/archea.gb"
            )  # some archea DNA
    ...
    if (
        refSeqFile != None
    ):  # if user gave a reference file not in fasta, let's consider it to look for features
        if refSeqFile[-6:] != ".fasta":
            genBankReference = True
            refSeq = SeqIO.read(refSeqFile, "genbank")
            refSeqFileForGenes = refSeqFile

    circularCheck = circularizationCheck.circularizationCheck(
        "best_query.fasta", circularSize, circularOffSet, blastFolder
    )  # returns a tuple with True or False and coordinates
    ...
    checktRNA = tRNAscanChecker.tRNAscanCheck(
        "best_query.fasta",
        circularCheck,
        skipTrnaScan,
        organismType,
    ...
    if ignoreFirstBuildChecks == False:
        checktRNA.checkFeatures = geneChecker.geneCheck(
            refSeqFileForGenes,
            "best_query.fasta",
            cutoffEquality,
            genBankReference,
        ...
        print(
            "Features found: %s / %s"
            % (len(presentFeatures) - len(splitFeatures), len(importantFeatures))
        )
    else:
    ...
    if (len(checktRNA) == 21 and buildCloroplast == False) or (
        len(checktRNA) == 37 and buildCloroplast == True
    ):
        ...
        if circularCheck[0] == True:
            ...
            if len(presentFeatures) == len(importantFeatures):
                ...
                print(
                    "And all genes, tRNAs and rRNAs were found, but some were split or duplicated."
                )
            else:
                print(
                    "But not all genes, tRNAs and rRNAs were found, storing info and rebuilding..."
                )
                print("")
                return checktRNA
            print("")
...
def checkSoapOutput(
    ...
    if (
        refSeqFile != None
    ):  # if user gave a reference file, let's consider its size as sizeToLook
        if refSeqFile[-6:] != ".fasta":
            genBankReference = True
            refSeq = SeqIO.read(refSeqFile, "genbank")
    ...
    if usingSOAP == True:
        ...
        if numberOfReadGroups > 1:
            scafFile = (
                ...
                + "-denovo_d_results/"
                + processName
                + "-denovo_out_AllStrains.unpadded.fasta"
            )
        else:
            scafFile = (
                pathToWork
    ...
    if len(possible_sequences) == 0:
        print(
            "No possible sequences were found according to size check. Skipping the other checks."
        )
        print("")
        return False
        ...
        if blastFolder == "installed":
            command = (
                "formatdb -i " + blastFastaFile + " -p F"
            )  # need to formatdb refseq first
        else:
            command = (
                blastFolder + "/bin/makeblastdb -in " + blastFastaFile + " -dbtype nucl"
            )  # need to formatdb refseq first
        ...
        with open("possible_hits.blast.xml", "w") as blastResultFile:
            if blastFolder == "installed":
                ...
                command = (
                    blastFolder
                    + "/bin/blastn -task blastn -db "
                    + blastFastaFile
                    + " -query possible_hits.fasta -evalue "
                    + str(blasteVal)
                    + " -outfmt 5 -num_threads 2"
                )  # call BLAST with XML output
            args = shlex.split(command)
        ...
        blastparse = SearchIO.parse(
            "possible_hits.blast.xml", "blast-xml"
        )  # get all queries
        dictOfBlastResults = {}
        ...
        for record in SeqIO.parse(
            ...
            if record.id == qresult.id:  # found the best match
                ...
                if (
                    len(final_Record.seq) < sizeToLook * 0.925 and noExtension == False
                ):  # if lower than 92.5%, try to increase this sequence
                    print(
                        "Trying to find other contigs that match the target reference...\nSize before extension: ",
                        len(final_Record.seq),
                    )
                    ...
                    blastparse2 = SearchIO.parse(
                        "possible_hits.blast.xml", "blast-xml"
                    )  # open xml parse
                    for (
                        extendingResult
                    ) in blastparse2:  # in each query, let's check for span size
                        ...
                        if extendingResult.seq_len >= 150:
                            for (
                                ...
                                extendingResult.hsps
                            ):  # let's sum up all HSPs span sizes
                                if hsp_2.aln_span >= 150:
                                    # to add to the dictOfComplements, in case we needed to reverse this seq
                                    reverseComplement = (
                                        hsp_2.hit_range[0] > hsp_2.hit_range[1]
                                        or hsp_2.hit_frame == -1
                                    )
                                    lowestStart = min(
                                        hsp_2.hit_range[0], hsp_2.hit_range[1]
                                    )
                                    ...
                                    queryStart = min(
                                        hsp_2.query_range[0], hsp_2.query_range[1]
                                    )
                                    queryEnding = max(
                                        hsp_2.query_range[0], hsp_2.query_range[1]
                                    )
                        ...
                        if addToDict == True:
                            ...
                            if lowestStart not in dictOfStarts:
                                ...
                                if (
                                    extendingResult.seq_len
                                    > dictOfStarts[lowestStart].seq_len
                                ):
                                    dictOfStarts[lowestStart] = extendingResult
                                    dictOfEndings[lowestStart] = highestEnding
                                    dictOfComplements[lowestStart] = reverseComplement
					...
					"""
                    listOfValidResults = sorted(
                        dictOfStarts.keys()
                    )  # list of found contigs sorted based on start positions
                    numberOfReverseComplementsAdded = (
                        0  # if this is > 0 then reverse final result, otherwise, don't
                    )
                        ...
                        for record2 in SeqIO.parse(
                            ...
                            if record2.id == blastResult.id:
                                ...
                                if reverseComplement == True:
                                    ...
                                    seqToAppend = record2.seq.reverse_complement()[
                                        queryStart:queryEnd
                                    ]
                                else:
                                    numberOfReverseComplementsAdded -= 1
                                ...
                                if lastInsertEnding == 0:
                                    ...
                                    extraSeqsFound += (
                                        "n" * max(20, (startVal - lastInsertEnding))
                                        + seqToAppend
                                    )
                                elif startVal <= lastInsertEnding:
                                    seqToAppend = seqToAppend[
                                        (-1)
                                        * (dictOfEndings[startVal] - lastInsertEnding) :
                                    ]
                    ...
                    if len(extraSeqsFound) - extraSeqsFound.lower().count("n") > len(
                        final_Record.seq
                    ):
                        ...
                        print(
                            "Size after extension (without Ns): ",
                            len(final_Record.seq) - final_Record.seq.lower().count("n"),
                        )
                    else:
                        print(
                            "Extension was not possible, keeping original sequence.\n"
                        )
                ...
                return checkResults(
                    ...
                    blastFolder,
                    noExtension,
                    ignoreFirstBuildChecks,
                    coveCutOff,
                    buildBacteria,
                    buildArchea,
                    len(listOfValidResults),
                )
        ...
        print(
            "Possible hits were found. Grabbing sequence closest to optimum size... Going to next step..."
        )
        print("")
