...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="This script is a subpart of Mitofinder designed to create files for tbl2asn submission.",
        formatter_class=SmartFormatter,
    )
    parser.add_argument(
        "-i",
        "--index",
        help="Index file (comma-delimited table)",
        default="",
        dest="table",
    )
    parser.add_argument(
    ...
    for index, row in df.iterrows():
        ...
        for f in sorted(glob.glob(seq + "*.fasta")):
            ...
            if os.path.isfile(f.split(".fasta")[0] + ".tbl"):
                ...
                print(
                    "WARNING: no tbl files (.tbl) were found for "
                    + str(seqid)
                    + " at the following path:\n"
                    + str(path)
                )
                print(
                    'The annoation inforamtions for the contig "'
                    + str(f.split("/")[-1])
                    + '" will not be found in the final tbl file : "'
                    + str(f.split(".fasta")[0].split("/")[-1] + ".tbl")
                    + '"\n'
                )
            fout.close()
        if c == 0:
            print(
                "WARNING: no fasta files (.fasta) were found for "
                + str(seqid)
                + " at the following path:\n"
                + str(path)
                + "\n"
